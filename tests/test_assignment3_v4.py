...
############################### Docker.exe Linux Commands ###########################################################
def removeSubnet(subnetName):
    command = "docker.exe network rm " + subnetName
    os.system(command)
    time.sleep(2)


def createSubnet(subnetAddress, subnetName):
    command = "docker.exe network create --subnet=" + subnetAddress + " " + subnetName
    os.system(command)
    time.sleep(2)


def buildDockerImage():
    command = "docker.exe build -t assignment3-img ."
    os.system(command)


def runReplica(hostPort, ipAddress, subnetName, instanceName):
    command = "docker.exe run -d -p " + hostPort + ":8080 --net=" + subnetName + " --ip=" + ipAddress + " --name=" + instanceName + " -e SOCKET_ADDRESS=" + ipAddress + ":8080" + " -e VIEW=" + view + " assignment3-img"
    os.system(command)
    time.sleep(20)


def stopAndRemoveInstance(instanceName):
    stopCommand = "docker.exe stop " + instanceName
    removeCommand = "docker.exe rm " + instanceName
...
def connectToNetwork(subnetName, instanceName):
    command = "docker.exe network connect " + subnetName + " " + instanceName
    os.system(command)


def disconnectFromNetwork(subnetName, instanceName):
    command = "docker.exe network disconnect " + subnetName + " " + instanceName
    os.system(command)
...
class TestHW3(unittest.TestCase):
    ######################## Build docker.exe image and create subnet ################################
    print("###################### Building Dokcer image ######################\n")
    # build docker.exe image
    buildDockerImage()
    ...
    def test_a_view_operations(self):
        ...
        response = requests.put('http://localhost:8082/key-value-store/key1',
                                json={'value': "value1", "causal-metadata": ""})
        ...
        # put a new key in the store
        response = requests.put('http://localhost:8082/key-value-store/key2',
                                json={'value': "value2", "causal-metadata": ""})
        self.assertEqual(response.status_code, 201)
    ...
    def test_b_availability(self):
        ...
        # put a new key in the store
        response = requests.put('http://localhost:8082/key-value-store/key',
                                json={"value": "myvalue", "causal-metadata": ""})
        responseInJson = response.json()
    ...
    def test_c_key_value_operations(self):
        ...
        # put a new key in the store
        response = requests.put('http://localhost:8082/key-value-store/mykey1',
                                json={"value": "myvalue1", "causal-metadata": ""})
        responseInJson = response.json()
        ...
        # put a new key in the store
        response = requests.put('http://localhost:8082/key-value-store/mykey1',
                                json={"value": "myvalue2", "causal-metadata": third_get_causal_metadata})
        responseInJson = response.json()
    ...
    def test_d_causal_consistency(self):
        ...
        # put a new key in the store
        response = requests.put('http://localhost:8082/key-value-store/k1',
                                json={"value": "foo", "causal-metadata": ""})
        responseInJson = response.json()
        ...
        response = requests.put('http://localhost:8083/key-value-store/k1',
                                json={"value": "bar", "causal-metadata": first_put_causal_metadata})
        responseInJson = response.json()
